...
#         D_S = D_S.permute(1, 0, 2) #[N, T, C]
        ...
        if not (self.adj.shape[0] == 1 and self.adj.shape[1] == 1):
...
class STTransformer(nn.Module):
    def __init__(
        ...
        # 缩小通道数，降到out_channels维。
        self.conv3 = nn.Conv2d(embed_size, out_channels, 1)

        self.conv4 = nn.Conv2d(N_in, N_out, 1)
...
#         output_Transformer = output_Transformer.unsqueeze(0)     
        ...
        out = self.conv3(out)                   # 等号左边 out shape: [B, C_out, N, output_T_dim]   
        
        out = out.permute(0, 2, 1, 3)           # out shape: [B, N, C_out, output_T_dim]
        out = self.conv4(out)                   # out shape: [B, N_out, C_out, output_T_dim]
        
       
        return out #[B, N_out, C_out , output_dim]
