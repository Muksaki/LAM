...
def load_graphdata_channel1(dataset_name, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True):
    '''
    这个是为PEMS的数据准备的函数
...
def load_graphdata_channel_my(filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True):
    '''
    这个是为PEMS的数据准备的函数
    将x,y都处理成归一化到[-1,1]之前的数据;
    每个样本同时包含所有监测点的数据，所以本函数构造的数据输入时空序列预测模型；
...
def image_to_patches(images, patch_size=16):
    """
    Convert images to patches.

    :param images: A batch of images of shape [batch_size, seq_len, height, width, channels].
    :param patch_size: The size of one side of the square patch (default is 16).

    :return: Patches of shape [batch_size, seq_len, num_patches, patch_size*patch_size*channels].
    """
    ...
    # Reshape images for patch extraction
    # New shape will be [batch_size, seq_len, num_patches_height, patch_size, num_patches_width, patch_size, channels]
    images_reshaped = images.reshape(
        batch_size, seq_len, height // patch_size, patch_size, width // patch_size, patch_size, channels
    )

    # Transpose to group patches together
    # New shape will be [batch_size, seq_len, num_patches_height, num_patches_width, patch_size, patch_size, channels]
    images_transposed = images_reshaped.permute(0, 1, 2, 4, 3, 5, 6)

    # Flatten to get the final shape [batch_size, seq_len, num_patches, patch_size*patch_size*channels]
    patches = images_transposed.reshape(batch_size, -1, patch_size * patch_size * channels, seq_len)
